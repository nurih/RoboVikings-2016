package org.firstinspires.ftc.teamcode;

/**
 * This class has some things that the whole team uses.
 * Instead of copying things like strings and numbers, we stick them here and refer to them
 * from our individual programs.
 **/
public class TeamShared {
    public static final String VisionKey = "";
}
